...
def randomforest_classifier(user_id,session,model_id, dataset_id, x_train, y_train, x_test, y_test, n_iter, optimizer="BayesianOptimization",param_space=None,task="classification"):
    global id_model 
    global id_dataset
    global iterations
    global session_id
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        try:
            ...
            plt.savefig(f"{session}/{model_id}/permutation_test_score.png")
            mlflow.log_artifact(f"{session}/{model_id}/permutation_test_score.png")
            ...
            # Log the model
            registered_model_name="random_forest_classification_model_"+str(timestamp) #FIXME: why logistic

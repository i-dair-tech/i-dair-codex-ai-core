...
def logistic_regression(user_id,session,model_id,dataset_id,x_train,y_train,x_test,y_test,n_iter,optimizer="BayesianOptimization",param_space=None,task="classification"):
    global id_model 
    global id_dataset
    global iterations
    global session_id
    ...
    folds_number_existance="folds_number" in param_space
    if folds_number_existance:
    ...
    if optimizer=="RandomSearch":
        ...
        search = BayesSearchCV(model, param_space, n_iter=n_iter, n_jobs=-1, cv=folds_number, random_state=123,
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        ...
        try:
            ...
            registered_model_name="logistic_Regression_model_"+str(timestamp)
            # Log the model

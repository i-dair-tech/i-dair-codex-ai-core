...
def randomforest_regression(user_id,session,model_id, dataset_id, x_train, y_train, x_test, y_test, n_iter, optimizer="BayesianOptimization",param_space=None):
    global id_model 
    global id_dataset
    global iterations
    global session_id
    ...
    # Define the number of bootstrap samples for the test
    nperms = 100
    ...
    experiment_name = 'Random Forest Regression Experiment '+ str(timestamp)
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        try:
            ...
            bootstrap_estimates = bootstrap(model=best_model, X=x_test, y=y_test, metrics=scores, model_name="Random Forest", nreps=nperms)
            ...
            registered_model_name = "random_forest_regression_model_" + str(timestamp)
            # Log the model
